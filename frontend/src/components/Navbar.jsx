  ...
  return (
    <header className={`fixed top-0 inset-x-0 z-50 transition-colors duration-500 ${shell}`}> 
      <div className="container flex items-center justify-between py-4">
        <a href="#top" className="relative text-sm font-bold uppercase tracking-[0.65em] bg-gradient-to-r from-amber-700 via-amber-500 to-amber-300 bg-clip-text text-transparent" onClick={close}>
          OlfactiveEcho
        ...
        <nav className="hidden md:flex items-center gap-8" aria-label="Main navigation">
          {navLinks.map(l => (
            <a key={l.href} href={l.href} onClick={close} className="relative text-[13px] font-semibold uppercase text-neutral-700 hover:text-neutral-900 transition-colors py-1">
              {l.label}
      ...
      <div className={`md:hidden fixed inset-y-0 right-0 w-2/3 max-w-xs bg-white/95 backdrop-blur-xl border-l border-neutral-200 transition-transform duration-700 ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col pt-28 pb-10 px-8 gap-8">
          {navLinks.map(l => (
            <a key={l.href} href={l.href} onClick={close} className="text-[14px] font-semibold uppercase text-neutral-700 hover:text-neutral-900 transition-colors relative group">
              {l.label}
